...
class {{feature_name.pascalCase()}}Cubit extends Cubit<{{feature_name.pascalCase()}}State> {
  ...
  Future<void> {{method_name}}({{params_type.pascalCase()}} params) async {
  ...
  void debounced{{method_name.pascalCase()}}({{params_type.pascalCase()}} params, {Duration duration = const Duration(milliseconds: 300)}) {
    _debounceTimer?.cancel();
    _debounceTimer = Timer(duration, () => {{method_name}}(params));
  }
