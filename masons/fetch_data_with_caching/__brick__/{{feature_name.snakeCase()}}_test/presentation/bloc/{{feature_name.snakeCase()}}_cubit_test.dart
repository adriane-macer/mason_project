...
void main() {
  ...
  setUp(() {
    ...
    registerFallbackValue(GetWalletUseCaseParams());
  });
  ...
  blocTest<{{feature_name.pascalCase()}}Cubit, {{feature_name.pascalCase()}}State>(
    ...
    build: () {
      when(() => useCase(any())).thenAnswer((_) => Stream.value(right(const {{return_type.pascalCase()}}Model())));
      return cubit;
    },
  ...
  blocTest<{{feature_name.pascalCase()}}Cubit, {{feature_name.pascalCase()}}State>(
    ...
    build: () {
      when(() => useCase(any())).thenAnswer((_) => Stream.value(left(const ServerFailure(message: 'error'))));
      return cubit;
    },
    act: (cubit) => cubit.{{method_name}}(),
    expect: () => [
